...
class CommonDesignLogic(object):
    ...
    def call_3DModel(self, status, connection=None):
        if connection:
            self.module_object = connection
        if status:
            self.display_3DModel("Model", "gradient_light")
    ...
    def display_3DModel(self, component, bgcolor):
        ...
        # Clear stale AIS object references to prevent crashes on repeated display calls

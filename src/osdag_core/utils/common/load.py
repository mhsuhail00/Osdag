class Load(object):

    def __init__(self,
                 axial_force=0.0,
                 shear_force=0.0,
                 shear_force_zz=0.0,
                 shear_force_yy=0.0,
                 moment=0.0,
                 moment_zz=0.0,
                 moment_yy=0.0,
                 moment_minor=0.0,
                 unit_kNm=False):

        force_multiplier = 1.0
        moment_multiplier = 1.0
        if unit_kNm is True:
            force_multiplier = 1e3
            moment_multiplier = 1e6
        print(force_multiplier, "is force multiplier")
        
        # Convert empty strings or None to 0
        axial_force = 0 if (axial_force == "" or axial_force is None) else axial_force
        shear_force = 0 if (shear_force == "" or shear_force is None) else shear_force
        shear_force_zz = 0 if (shear_force_zz == "" or shear_force_zz is None) else shear_force_zz
        shear_force_yy = 0 if (shear_force_yy == "" or shear_force_yy is None) else shear_force_yy
        moment = 0 if (moment == "" or moment is None) else moment
        moment_zz = 0 if (moment_zz == "" or moment_zz is None) else moment_zz
        moment_yy = 0 if (moment_yy == "" or moment_yy is None) else moment_yy
        moment_minor = 0 if (moment_minor == "" or moment_minor is None) else moment_minor
        
        # Now safely convert to float and multiply
        self.axial_force = force_multiplier * float(axial_force)
        
        # Shear forces
        self.shear_force = force_multiplier * float(shear_force)
        self.shear_force_zz = force_multiplier * float(shear_force_zz)
        self.shear_force_yy = force_multiplier * float(shear_force_yy)
        
        # Moments
        self.moment = moment_multiplier * float(moment)
        self.moment_yy = moment_multiplier * float(moment_yy)
        self.moment_zz = moment_multiplier * float(moment_zz)
        self.moment_minor = moment_multiplier * float(moment_minor)
        
        print("setting factored input loads as, axial force = {0} N, shear force = {1} N, moment = {2} Nmm".format(
            self.axial_force, self.shear_force, self.moment))

    def __repr__(self):
        repr = "Load\n"
        repr += "Axial Force: {}\n".format(self.axial_force)
        repr += "Shear Force: {}\n".format(self.shear_force)
        repr += "Moment: {}\n".format(self.moment)
        repr += "Moment Minor: {}\n".format(self.moment_minor)
        return repr
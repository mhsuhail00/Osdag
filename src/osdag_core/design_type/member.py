...
class Member(Main):
    ...
    def tab_strut_channel_section(self, input_dictionary):
        ...
        if not input_dictionary or input_dictionary[KEY_SECSIZE] == [] or \
            ...
            if section_profile == "Channels":
                ...
                It = str(Channel_attributes.calc_TorsionConstantIt(f_w, f_t, w_h, w_t))
                Iw = "-"
        print(f"tab_strut_channel_section input_dictionary {input_dictionary}")
        if KEY_SEC_MATERIAL in input_dictionary.keys():
            material_grade = input_dictionary[KEY_SEC_MATERIAL]
            material_attributes = Material(material_grade)
            fu = material_attributes.fu
    ...
    def optimization_tab_flexure_design(self, input_dictionary):
        ...
        if KEY_MODULE in input_dictionary:
            if input_dictionary[KEY_MODULE] == KEY_DISP_FLEXURE:
                t9 = ("textBrowser", "", TYPE_TEXT_BROWSER, FLEXURE_OPTIMIZATION_DESCRIPTION_SimplySupp , None)
                optimum.append(t9)
            else:
                t9 = ("textBrowser", "", TYPE_TEXT_BROWSER, FLEXURE_OPTIMIZATION_DESCRIPTION_Canti , None)
                optimum.append(t9)

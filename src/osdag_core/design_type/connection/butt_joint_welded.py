...
class ButtJointWelded(MomentConnection):
    ...
    def detailing_values(self, input_dictionary):
        ...
        t4 = ("textBrowser", "", TYPE_TEXT_BROWSER,
              DETAILING_DESCRIPTION_LAPJOINT, None)
        detailing.append(t4)
    ...
    def input_values(self):
        ...
        t16 = (KEY_MODULE, KEY_DISP_BUTTJOINTWELDED,
        ...
        t31 = (KEY_PLATE1_THICKNESS, KEY_DISP_PLATE1_THICKNESS,
               TYPE_COMBOBOX, VALUES_PLATETHK_CUSTOMIZED, True, 'Int Validator')
        ...
        t6 = (KEY_MATERIAL, KEY_DISP_MATERIAL, TYPE_COMBOBOX,
              VALUES_MATERIAL, True, 'No Validator')
        options_list.append(t6)

        t36 = (KEY_COVER_PLATE, KEY_DISP_COVER_PLT, TYPE_COMBOBOX,
               VALUES_COVER_PLATE, True, 'No Validator')
        options_list.append(t36)

        t18 = (None, DISP_TITLE_WELD, TYPE_TITLE, None, True, 'No Validator')
        options_list.append(t18)

        t20 = (KEY_WELD_SIZE, KEY_DISP_WELD_SIZE, TYPE_COMBOBOX_CUSTOMIZED,
               VALUES_ALL_CUSTOMIZED, True, 'No Validator')
    ...
    def spacing(self, status):
        ...
        t111 = (KEY_OUT_GAUGE, KEY_OUT_DISP_GAUGE, TYPE_TEXTBOX,
    ...
    def output_values(self, flag):
        ...
        plate2_thk = float(self.plate2.thickness[0]) if hasattr(
            self, 'plate2') and self.plate2 and self.plate2.thickness else 0
    ...
    def design_of_weld(self, design_dictionary):
        ...
        if isinstance(weld_size, str) and weld_size.lower() == 'all':
            ...
            if isinstance(values_to_process, list):
                ...
                valid_custom = [
                    s for s in float_weld_sizes if s_min <= s <= s_max]
                if valid_custom:
        ...
        if self.weld_size is None:
            self.logger.error(
                ": weld_size is not set. Cannot proceed with weld design.")
            self.design_status = False
            self.logger.error(
                ": Design status: UNSAFE due to missing or invalid weld size.")
    ...
    def weld_length(self, design_dictionary):
        ...
        self.s_min = IS800_2007.cl_10_5_2_3_min_weld_size(
        ...
        if self.weld_size < self.s_min or self.weld_size > self.s_max:
            ...
            if self.weld_size < self.s_min:
                ...
                self.logger.info(
                    ": Design action required: Increase the weld size to at least {} mm".format(self.s_min))
            else:
                self.logger.error(": Weld size fails: Size {} mm exceeds maximum allowed {} mm".format(
                    self.weld_size, self.s_max))
                self.logger.info(
                    ": Design action required: Decrease the weld size to at most {} mm".format(self.s_max))
        ...
        self.f_w = self.fu / (math.sqrt(3) * self.gamma_mw)
        ...
        self.L_req = self.tensile_force / \
            (self.N_f * 0.707 * self.weld_size * self.f_w)
        ...
        if self.L_req <= self.plates_width:
            ...
            self.weld_angle = math.degrees(
                math.atan((L_target - self.plates_width)/(2 * self.plates_width)))
            ...
            L_provided_line = self.plates_width + 2 * \
                self.plates_width * math.tan(math.radians(self.weld_angle))
            ...
            if L_provided_total < self.L_req:
                ...
                # As per IS 800:2007 Cl 10.5.10.2
                min_return = max(2 * self.weld_size, 10)
            ...
            self.weld_length_effective = L_provided_total + \
                (2 * self.side_weld_length * self.N_f)
    ...
    def weld_strength_verification(self, design_dictionary):
        ...
        if not hasattr(self, 'weld_length_provided'):
            self.logger.error(
                ": Weld length must be calculated before strength verification")
            self.design_status = False
            return
        ...
        self.weld_strength = self.f_w * 0.707 * \
            self.weld_size * self.weld_length_effective * self.N_f
    ...
    def long_joint_reduction_factor(self):
        ...
        if self.weld_length_effective <= 150 * a:
            self.beta_L = 1.0
            self.logger.info(
                ": No reduction for long joints required as length is less than 150 times throat thickness")
            return
        ...
        # Update the utilization ratio
        self.utilization_ratios['weld'] = weld_utilization_reduced
    ...
    def check_base_metal_strength(self, design_dictionary):
        ...
        try:
            ...
            self.logger.error(
                f": Invalid material grade format: {material_grade}")
            self.design_status = False
            return
        ...
        if base_metal_utilization > 1:
            if self.design_for == 'Compression':
                self.logger.error(
                    ": Base metal strength in compression is insufficient [cl. 7, IS 800:2007]")
            else:
                self.logger.error(
                    ": Base metal strength in tension is insufficient [cl. 6, IS 800:2007]")
        else:
            if self.design_for == 'Compression':
                self.logger.info(
                    ": Base metal strength in compression is adequate")
            else:
                self.logger.info(
                    ": Base metal strength in tension is adequate")
    ...
    def calculate_final_utilization_ratio(self):
        ...
        if not hasattr(self, 'utilization_ratios'):
            self.logger.error(
                ": Cannot calculate final utilization ratio - component checks incomplete")
            self.design_status = False
            return
        ...
        if self.utilization_ratio > 1.0:
            ...
            critical_component = max(
                self.utilization_ratios.items(), key=lambda x: x[1])[0]
            ...
            self.logger.info(
                ": All utilization ratios are within acceptable limits")
    ...
    def save_design(self, popup_summary):
        ...
        try:
            ...
            # Plate properties
            plate1_thk = f2(self.plate1.thickness[0] if isinstance(
                self.plate1.thickness, list) else self.plate1.thickness, 0.0)
            plate2_thk = f2(self.plate2.thickness[0] if isinstance(
                self.plate2.thickness, list) else self.plate2.thickness, 0.0)
            ...
            # Load
            axial_force_N = f2(
                g('axial_force', g('axialforce', g('tensileforce', 0.0))), 0.0)
            axial_kN = f2(axial_force_N / 1000, 0.0)

            # Cover plate
            cover_plate_str = g('cover_plate', g(
                'cover_plate_type', 'Single-Cover'))
            N_f = 2 if "double" in cover_plate_str.lower() else 1
            cover_thk = f2(g('calculated_cover_plate_thickness', 0.0), 0.0)
            packing_thk = f2(g('packing_plate_thickness',
                             g('packing_thickness', 0.0)), 0.0)
            ...
            weld_strength_N = f2(
                f_w_adjusted * effective_throat * L_eff_provided * N_f, 0.0)
            ...
            tcp_calc.append(NoEscape(r'T_{cp} &\geq \frac{' + str(
                tcp_numerator) + r'}{' + str(tcp_denominator) + r'} \times T_{min}\\'))
            tcp_calc.append(NoEscape(r'&\geq \frac{' + str(tcp_numerator) + r'}{' + str(
                tcp_denominator) + r'} \times ' + str(plate_thk_min) + r'\\'))
            tcp_calc.append(
                NoEscape(r'&\geq ' + str(tcp_req) + r' \text{ mm}\\'))
            tcp_calc.append(NoEscape(r'\end{aligned}'))

            tcp_prov = Math(inline=True)
            tcp_prov.append(
                NoEscape(r'T_{cp} = ' + str(cover_thk) + r' \text{ mm}'))
            ...
            if abs(plate1_thk - plate2_thk) > 0.001 and N_f == 2:
                ...
                packing_calc.append(
                    NoEscape(r'&= |' + str(plate1_thk) + r' - ' + str(plate2_thk) + r'|\\'))
                packing_calc.append(
                    NoEscape(r'&= ' + str(packing_thk) + r' \text{ mm}\\'))
                packing_calc.append(NoEscape(r'\end{aligned}'))

                packing_prov = Math(inline=True)
                packing_prov.append(
                    NoEscape(str(packing_thk) + r' \text{ mm}'))

                self.report_check.append(
                    ["Packing Plate", packing_calc, packing_prov, "PASS"])
            ...
            s_min = IS800_2007.cl_10_5_2_3_min_weld_size(
                plate1_thk, plate2_thk)
            s_max = f2(plate_thk_min - 1.5, 0.0)

            size_check_req = Math(inline=True)
            size_check_req.append(NoEscape(r'\begin{aligned}'))
            size_check_req.append(
                NoEscape(r's_{min} &= ' + str(s_min) + r' \text{ mm}\\'))
            size_check_req.append(
                NoEscape(r'&[\text{Table 21, IS 800:2007}]\\'))
            size_check_req.append(NoEscape(r's_{max} &= T_{min} - 1.5\\'))
            size_check_req.append(
                NoEscape(r'&= ' + str(plate_thk_min) + r' - 1.5\\'))
            size_check_req.append(
                NoEscape(r'&= ' + str(s_max) + r' \text{ mm}\\'))
            size_check_req.append(
                NoEscape(r'&[\text{Ref. Cl. 10.5.2.3, 10.5.2.4}]'))
            size_check_req.append(NoEscape(r'\end{aligned}'))

            size_check_prov = Math(inline=True)
            size_check_prov.append(
                NoEscape(r's = ' + str(weld_size) + r' \text{ mm}'))

            size_status = "PASS" if (s_min <= weld_size <= s_max) else "FAIL"
            self.report_check.append(
                ["Weld Size Check", size_check_req, size_check_prov, size_status])
            ...
            throat_calc.append(
                NoEscape(r'&= 0.707 \times ' + str(weld_size) + r'\\'))
            throat_calc.append(
                NoEscape(r'&= ' + str(effective_throat) + r' \text{ mm}\\'))
            ...
            weld_strength_calc.append(
                NoEscape(r'f_{wd} &= \frac{f_u}{\sqrt{3} \times \gamma_{mw}}\\\\'))
            weld_strength_calc.append(NoEscape(
                r'&= \frac{' + str(f2(fu_weld)) + r'}{\sqrt{3} \times ' + str(gamma_mw) + r'}\\\\'))
            weld_strength_calc.append(
                NoEscape(r'&= ' + f'{f_w:.2f}' + r' \text{ N/mm}^2\\'))
            weld_strength_calc.append(NoEscape(r'\end{aligned}'))

            self.report_check.append(
                ["Design Strength", "", weld_strength_calc, ""])
            ...
            length_req_calc.append(
                NoEscape(r'L_{req} &= \frac{P_N}{f_{wd} \times a \times n_f}\\\\'))
            length_req_calc.append(NoEscape(r'&= \frac{' + str(int(axial_force_N)) + r'}{' +
                                   f'{f_w:.2f}' + r' \times ' + str(effective_throat) + r' \times ' + str(N_f) + r'}\\\\'))
            length_req_calc.append(
                NoEscape(r'&= ' + str(L_req) + r' \text{ mm}\\\\'))
            length_req_calc.append(
                NoEscape(r'&[\text{Ref. Section 3.3, DDCL}]'))
            ...
            length_condition.append(
                NoEscape(r'' + str(L_req) + r' &\leq ' + str(width) + r'\\'))
            if L_req <= width:
                length_condition.append(
                    NoEscape(r'&\text{Straight weld is sufficient}'))
                req_status = "PASS"
            else:
                length_condition.append(
                    NoEscape(r'&\text{Proceed to Section 3.4}'))
                req_status = ""
            length_condition.append(NoEscape(r'\end{aligned}'))

            self.report_check.append(
                ["Required Length", length_req_calc, length_condition, req_status])
            ...
            if L_req > width:
                ...
                skew_calc_detail.append(
                    NoEscape(r'L_{target} &= \frac{L_{req}}{n_f}\\'))
                skew_calc_detail.append(
                    NoEscape(r'&= \frac{' + str(L_req) + r'}{' + str(N_f) + r'}\\'))
                skew_calc_detail.append(
                    NoEscape(r'&= ' + str(L_target) + r' \text{ mm}\\'))
                skew_calc_detail.append(
                    NoEscape(r'&[\text{Ref. Section 3.4.1, Step 1}]'))
                skew_calc_detail.append(NoEscape(r'\end{aligned}'))

                self.report_check.append(
                    ["Target Length/Line", "", skew_calc_detail, ""])

                # Calculate skew angle
                if skew_angle > 0:
                    skew_angle_calc = Math(inline=True)
                    skew_angle_calc.append(NoEscape(r'\begin{aligned}'))
                    skew_angle_calc.append(
                        NoEscape(r'\alpha &= \arctan\left(\frac{L_{target} - w}{2w}\right)\\'))
                    skew_angle_calc.append(NoEscape(r'&= \arctan\left(\frac{' + str(
                        L_target) + r' - ' + str(width) + r'}{2 \times ' + str(width) + r'}\right)\\'))
                    skew_angle_calc.append(
                        NoEscape(r'&= ' + f'{skew_angle:.1f}' + r'^\circ\\'))

                    # Check constraints 20° ≤ α ≤ 60°
                    if skew_angle < 20:
                        skew_angle_calc.append(NoEscape(
                            r'&\text{Since } \alpha < 20^\circ, \text{ set } \alpha = 20^\circ\\'))
                    elif skew_angle > 60:
                        skew_angle_calc.append(NoEscape(
                            r'&\text{Since } \alpha > 60^\circ, \text{ set } \alpha = 60^\circ\\'))
                        skew_angle_calc.append(
                            NoEscape(r'&\text{(Proceed to Section 3.4.3)}\\'))

                    skew_angle_calc.append(
                        NoEscape(r'&[\text{Ref. Section 3.4.1, Steps 3-4}]'))
                    skew_angle_calc.append(NoEscape(r'\end{aligned}'))

                    self.report_check.append(
                        ["Skew Angle", skew_angle_calc, f'{skew_angle:.1f}°', ""])

                    # 3.4.2: Provided weld length per line
                    L_prov_line_calc = Math(inline=True)
                    L_prov_line_calc.append(NoEscape(r'\begin{aligned}'))
                    L_prov_line_calc.append(
                        NoEscape(r'L_{provided\_line} &= w + 2w \times \tan(\alpha)\\'))
                    L_prov_line_calc.append(NoEscape(r'&= ' + str(width) + r' + 2 \times ' + str(
                        width) + r' \times \tan(' + f'{skew_angle:.1f}' + r'^\circ)\\'))
                    L_prov_line_calc.append(
                        NoEscape(r'&= ' + str(L_provided_line) + r' \text{ mm}\\'))
                    L_prov_line_calc.append(
                        NoEscape(r'&[\text{Ref. Section 3.4.2, Step 1}]'))
                    L_prov_line_calc.append(NoEscape(r'\end{aligned}'))

                    self.report_check.append(
                        ["Provided Length/Line", "", L_prov_line_calc, ""])
                ...
                if side_weld_len > 0:
                    ...
                    side_calc.append(
                        NoEscape(r'L_{side} &= \frac{L_{req} - L_{provided}}{n_f}\\'))
                    side_calc.append(NoEscape(
                        r'&= \frac{' + str(L_req) + r' - ' + str(L_provided_total) + r'}{' + str(N_f) + r'}\\'))
                    side_calc.append(
                        NoEscape(r'&= ' + str(side_weld_len) + r' \text{ mm}\\'))

                    # Minimum return weld
                    min_return = max(2 * weld_size, 10)
                    side_calc.append(NoEscape(
                        r'\text{Min. return} &= \max(2s, 10) = ' + str(min_return) + r' \text{ mm}\\'))
                    side_calc.append(NoEscape(r'&[\text{Ref. Cl. 10.5.10.2}]'))
                    side_calc.append(NoEscape(r'\end{aligned}'))

                    self.report_check.append(
                        ["Side Weld Length", side_calc, f'{side_weld_len:.1f} mm', ""])
            ...
            if L_eff_provided > 150 * effective_throat:
                ...
                beta_req.append(
                    NoEscape(r'\text{Since } L_{eff} &> 150 \times a\\'))
                beta_req.append(NoEscape(
                    r'' + str(L_eff_provided) + r' &> 150 \times ' + str(effective_throat) + r'\\'))
                beta_req.append(NoEscape(
                    r'' + str(L_eff_provided) + r' &> ' + f'{150 * effective_throat:.1f}' + r'\\'))
                beta_req.append(
                    NoEscape(r'\beta_L &= 1.2 - \frac{0.2 \times L_{eff}}{150 \times a}\\'))
                beta_req.append(NoEscape(r'&= 1.2 - \frac{0.2 \times ' + str(
                    L_eff_provided) + r'}{150 \times ' + str(effective_throat) + r'}\\'))
                beta_calc_val = 1.2 - \
                    (0.2 * L_eff_provided) / (150 * effective_throat)
                beta_req.append(
                    NoEscape(r'&= ' + f'{beta_calc_val:.3f}' + r'\\'))
                beta_req.append(
                    NoEscape(r'&\text{(Ensure } \beta_L \geq 0.8\text{)}\\'))
                beta_req.append(
                    NoEscape(r'\beta_L &= ' + f'{beta_L:.2f}' + r'\\'))
                ...
                beta_req.append(NoEscape(r'\begin{aligned}'))
                beta_req.append(
                    NoEscape(r'\text{Since } L_{eff} &\leq 150 \times a\\'))
                beta_req.append(NoEscape(r'' + str(L_eff_provided) +
                                r' &\leq 150 \times ' + str(effective_throat) + r'\\'))
                beta_req.append(NoEscape(r'' + str(L_eff_provided) +
                                r' &\leq ' + f'{150 * effective_throat:.1f}' + r'\\'))
            ...
            self.report_check.append(
                ["Reduction Factor", beta_req, beta_prov, beta_status])
            ...
            weld_cap_calc.append(
                NoEscape(r'C_w &= f_{wd}^{adj} \times a \times L_{eff} \times n_f\\\\'))
            weld_cap_calc.append(
                NoEscape(r'&= (\beta_L \times f_{wd}) \times a \times L_{eff} \times n_f\\\\'))
            weld_cap_calc.append(NoEscape(r'&= (' + f'{beta_L:.2f}' + r' \times ' + f'{f_w:.2f}' + r') \times ' + str(
                effective_throat) + r' \times ' + str(L_eff_provided) + r' \times ' + str(N_f) + r'\\\\'))
            weld_cap_calc.append(
                NoEscape(r'&= ' + str(weld_strength_kN) + r' \text{ kN}\\'))
            ...
            if is_comp:
                ...
                comp_calc_req.append(
                    NoEscape(r'T_{db} &= \frac{A_g \times f_y}{\gamma_{m0}}\\'))
                comp_calc_req.append(NoEscape(
                    r'T_{db} &= \frac{' + f'{Ag:.1f}' + r' \times ' + str(fy) + r'}{' + str(gamma_m0) + r'}\\'))
                comp_calc_req.append(
                    NoEscape(r'&= ' + str(base_metal_capacity_kN) + r' \text{ kN}\\'))
                comp_calc_req.append(
                    NoEscape(r'&[\text{Ref. Section 3.7, Cl. 7.1.2}]'))
                comp_calc_req.append(NoEscape(r'\end{aligned}'))

                comp_status = "PASS" if base_metal_capacity_kN >= axial_kN else "FAIL"
                self.report_check.append(
                    ["Base Metal Capacity", axial_kN, comp_calc_req, comp_status])
            else:
                # For tension - show both calculations and take min
                ten_calc_req = Math(inline=True)
                ten_calc_req.append(NoEscape(r'\begin{aligned}\\'))
                ten_calc_req.append(NoEscape(
                    r'T_{db} &= \min\left(\frac{A_g f_y}{\gamma_{m0}}, \frac{0.9 A_n f_u}{\gamma_{m1}}\right)\\'))
                ten_calc_req.append(NoEscape(r'&= \min\left(\frac{' + f'{Ag:.1f}' + r' \times ' + str(fy) + r'}{' + str(
                    gamma_m0) + r'}, \frac{0.9 \times ' + f'{Ag:.1f}' + r' \times ' + str(fu) + r'}{' + str(gamma_m1) + r'}\right)\\'))
                ten_calc_req.append(
                    NoEscape(r'&= \min(' + str(Tdg) + r', ' + str(Tdn) + r')\\'))
                ten_calc_req.append(
                    NoEscape(r'&= ' + str(base_metal_capacity_kN) + r' \text{ kN}\\'))
                ten_calc_req.append(
                    NoEscape(r'&[\text{Ref. Section 3.7, Cl. 6.2, 6.3}]'))
                ten_calc_req.append(NoEscape(r'\end{aligned}'))

                ten_status = "PASS" if base_metal_capacity_kN >= axial_kN else "FAIL"
                self.report_check.append(
                    ["Base Metal Capacity", axial_kN, ten_calc_req, ten_status])
            ...
            detail_check_1.append(
                NoEscape(r'' + str(L_eff_provided) + r' &\geq ' + str(4 * weld_size)))
            detail_check_1.append(NoEscape(r'\end{aligned}'))
            detail_status_1 = "PASS" if L_eff_provided >= 4 * weld_size else "FAIL"
            self.report_check.append(
                ["Min. Eff. Length", detail_check_1, "", detail_status_1])
            ...
            detail_check_2.append(
                NoEscape(r'' + str(weld_size) + r' &\geq ' + str(s_min)))
            detail_check_2.append(NoEscape(r'\end{aligned}'))
            detail_status_2 = "PASS" if weld_size >= s_min else "FAIL"
            self.report_check.append(
                ["Min. Weld Size", detail_check_2, "", detail_status_2])
            ...
            detail_check_3.append(
                NoEscape(r'' + str(weld_size) + r' &\leq ' + str(s_max)))
            detail_check_3.append(NoEscape(r'\end{aligned}'))
            detail_status_3 = "PASS" if weld_size <= s_max else "FAIL"
            self.report_check.append(
                ["Max. Weld Size", detail_check_3, "", detail_status_3])
            ...
            if skew_angle > 0:
                ...
                detail_check_4.append(
                    NoEscape(r'20^\circ &\leq \alpha \leq 60^\circ\\'))
                detail_check_4.append(
                    NoEscape(r'20^\circ &\leq ' + f'{skew_angle:.1f}' + r'^\circ \leq 60^\circ'))
                detail_check_4.append(NoEscape(r'\end{aligned}'))
                detail_status_4 = "PASS" if 20 <= skew_angle <= 60 else "FAIL"
                self.report_check.append(
                    ["Skew Angle Range", detail_check_4, "", detail_status_4])

            # Check 5: Packing plate requirement
            if N_f == 2:
                if abs(plate1_thk - plate2_thk) > 0.001:
                    detail_check_5 = NoEscape(
                        r'\text{Required: } |t_1 - t_2| > 0')
                    detail_prov_5 = f"Provided: {packing_thk} mm"
                    detail_status_5 = "PASS"
                else:
                    detail_check_5 = NoEscape(
                        r'\text{Not required: } t_1 = t_2')
                    detail_prov_5 = "N/A"
                    detail_status_5 = "PASS"
                self.report_check.append(
                    ["Packing Plate", detail_check_5, detail_prov_5, detail_status_5])
            ...
            conn_len_detail.append(
                NoEscape(r'L_{provided} &= n_f \times L_{provided\_line}\\'))
            conn_len_detail.append(
                NoEscape(r'&= ' + str(N_f) + r' \times ' + str(L_provided_line) + r'\\'))
            L_provided_total = f2(N_f * L_provided_line, 0.0)
            conn_len_detail.append(
                NoEscape(r'&= ' + str(L_provided_total) + r' \text{ mm}'))
            conn_len_detail.append(NoEscape(r'\end{aligned}'))
            self.report_check.append(
                ["Connection Length", "", conn_len_detail, ""])
            ...
            cover_len_detail.append(
                NoEscape(r'L_{cp} &= L_{provided} + 2 \times (\text{end clearance})\\'))
            cover_len_detail.append(NoEscape(
                r'&= ' + str(L_provided_total) + r' + 2 \times ' + str(end_clearance) + r'\\'))
            cover_len_detail.append(
                NoEscape(r'&= ' + str(L_cover_plate) + r' \text{ mm}'))
            cover_len_detail.append(NoEscape(r'\end{aligned}'))
            self.report_check.append(
                ["Cover Plate Length", "", cover_len_detail, ""])
            ...
            utilization_ratio = f2(
                axial_kN / overall_capacity_kN if overall_capacity_kN > 0 else 0, 0.0)

            ur_calc = Math(inline=True)
            ur_calc.append(NoEscape(r'\begin{aligned}'))
            ur_calc.append(NoEscape(r'UR &= \frac{P_N}{\min(C_w, T_{db})}\\'))
            ur_calc.append(NoEscape(r'&= \frac{' + str(axial_kN) + r'}{\min(' + str(
                weld_strength_kN) + r', ' + str(base_metal_capacity_kN) + r')}\\'))
            ur_calc.append(NoEscape(
                r'&= \frac{' + str(axial_kN) + r'}{' + str(overall_capacity_kN) + r'}\\'))
            ...
            self.report_check.append(
                ["Utilization Ratio", f"{axial_kN:.2f} kN", ur_calc, ur_status])
            ...
            fname_no_ext = popup_summary.get(
                "filename", "ButtJointWeldedReport")
            ...
            self.logger.info(
                f"Report generated successfully: {fname_no_ext}.pdf")
            return True
